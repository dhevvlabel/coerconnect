  ...
  app.post("/api/planner/generate", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Primary model (gemini-3.5-flash) overloaded or failed, falling back to gemini-3.1-flash-lite...", primaryError.message || primaryError);
        try {
          response = await ai.models.generateContent({
          ...
          console.error("Failover model (gemini-3.1-flash-lite) also failed. Proceeding with high-fidelity local generator:", secondaryError);
