...
import { MutualMatch } from "./home/MutualMatch";
...
import { Users, LogOut, Shield, ShieldCheck, User as UserIcon, Globe, Trash2, Edit3, Check, X, ChevronUp, Instagram, Twitter, RefreshCw } from "lucide-react";
...
export function Dashboard() {
  ...
  const [privacy, setPrivacy] = useState({
    ...
    sharePlans: profile?.sharePlans !== false
  });
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'mutuals':
        return (
          <MutualMatch onBack={() => setActiveTab('home')} />
        );
      case 'settings':
        return (
                      ...
                      <div className="flex-1 text-center md:text-left space-y-1">
                        ...
                        {(profile?.instagram || profile?.twitter) && (
                          <div className="flex items-center justify-center md:justify-start gap-2 mt-2 flex-wrap">
                            {profile?.instagram && (
                              <span className="text-[9px] text-accent/60 font-black uppercase tracking-widest bg-accent/5 border border-accent/10 px-2.5 py-1 rounded-lg flex items-center gap-1">
                                <Instagram size={10} className="text-rose-500/80" /> {profile.instagram}
                              </span>
                            )}
                            {profile?.twitter && (
                              <span className="text-[9px] text-accent/60 font-black uppercase tracking-widest bg-accent/5 border border-accent/10 px-2.5 py-1 rounded-lg flex items-center gap-1">
                                <Twitter size={10} className="text-sky-500/80" /> {profile.twitter}
                              </span>
                            )}
                          </div>
                        )}
                ...
                <section className="pt-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <button 
                      ...
                      className="w-full py-6 rounded-[32px] bg-accent/5 border border-accent/10 text-accent font-black uppercase tracking-[0.3em] text-xs flex items-center justify-center gap-4 hover:bg-accent hover:text-surface transition-all duration-500 active:scale-95 shadow-2xl shadow-accent/5 group cursor-pointer"
                    >
                    ...
                    <button 
                      ...
                      className="w-full py-6 rounded-[32px] bg-red-500/10 border border-red-500/20 text-red-500 font-black uppercase tracking-[0.3em] text-xs flex items-center justify-center gap-4 hover:bg-red-500 hover:text-surface transition-all duration-500 active:scale-95 shadow-2xl shadow-red-500/5 group cursor-pointer"
                    >
              ...
              {/* Close Button */}
               ...
               <div className="space-y-4">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-100/40 pt-4">
                    <div className="space-y-1.5 text-left">
                      <label className="text-[9px] font-black uppercase text-slate-400 tracking-widest pl-1">Instagram @Handle</label>
                      <input 
                        value={editData.instagram}
                        onChange={(e) => setEditData({...editData, instagram: e.target.value})}
                        placeholder="e.g. @daffa_coer"
                        className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-3 px-4 text-xs font-bold text-slate-800 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 transition-all"
                      />
                    </div>

                    <div className="space-y-1.5 text-left">
                      <label className="text-[9px] font-black uppercase text-slate-400 tracking-widest pl-1">X / Twitter @Handle</label>
                      <input 
