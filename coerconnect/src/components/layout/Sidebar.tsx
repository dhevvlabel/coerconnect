...
export function Sidebar({ activeTab, setActiveTab }: SidebarProps) {
  const { profile, logout, switchAccount } = useAuth();
  const { t } = useLanguage();
    ...
    <aside className="hidden lg:flex w-72 border-r border-accent/10 bg-surface flex-col h-screen fixed left-0 top-0 z-[100]">
      ...
      <div className="p-6">
        ...
        <button 
          ...
          className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold text-accent/60 hover:text-accent hover:bg-sage/10 transition-colors uppercase tracking-tight mb-1"
        >
